 ...
 *  \brief  Contains the parameters to trigger an event of the form f = (dim - value) = 0, with dim = x, y or z.
 **/
...
struct value_params
{
    ...
    double value;
    /// Direction of events: if 1, only crosses from negative to positive values of f are kept. if -1, only crosses from positive to negative values of f are kept. if 0, all values are kept.
    int direction;
};
 ...
 *   Note that both the maximum number of events and the direction of events are already present in the structure value_params of the \c value_function. However, they are
 ...
 *   - The maximum number of events.
 *   - The direction of events.
 *   Note that both the maximum number of events and the direction of events are already present in the structure \c value_params of the value_function. However, they are
 *    also returned in the output in order to reproduce the MATLAB procedure for event detection.
 *   Note also that, given the structure value_params, only events of the form f(t, z, params)  = dim - value = 0, with dim = x, y, or z are possible.
 **/
 ...
 *  \brief  Structure dedicated to ode integration with an event detection via a \c value_function structure.
 **/
struct OdeParams
{
    ///Initial time
