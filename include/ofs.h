...
public:
     ...
     */
    void setCoef(T  const&  value, int const& pos);
    template <typename U> void setCoef(U const& value, int const& pos);
     ...
     *  \param pos: the position (indix in the serie) to modify
     */
    void addCoef(T  const&  value, int const& pos);
    /**
     *  \brief Sets a coefficient to all positions in the serie.
     *  \param value: the value to set
     */
    void setAllCoefs(T  const& value);
     ...
     *  \return the coefficient of type \c T at the position \c pos
     */
    T tfs_getCoef(int pos) const;

    /**
     *  \brief  Computes the maximum coefficient in norm.
     *  \param  maxAbx  a pointer to a \c double array of size 2, so that, for the serie \f$ F_s = \sum \limits_{k = -J}^{J} c_k e^{i k \theta }\f$,
     *   1. <tt>maxAbs[0]</tt> = \f$ \max \limits_{k \in [-J, J]} |c_k|\f$.
     *   2. <tt>maxAbs[1]</tt> = \f$ \arg\max \limits_{k \in [-J, J]} |c_k|\f$.
     */
    void getCoefMaxNorm(double maxAbs[]) const; //Get the maximum coefficient (in norm) + its indix

    /**
     *  \brief  Computes the maximum coefficient in norm.
     *  \return a \c double containing <tt>maxAbs[0]</tt> = \f$ \max \limits_{k \in [-J, J]} |c_k|\f$,
     *   for the serie \f$ F_s = \sum \limits_{k = -J}^{J} c_k e^{i k \theta }\f$.
     */
    double getCoefMaxNorm() const; //get only the maximum in norm
    ...
    */
     ...
     */
    void tfs_to_ofs(Ofs<T>& a, gsl_vector_complex *data_fft, gsl_fft_complex_wavetable *wavetable, gsl_fft_complex_workspace *workspace);
    ...
    //------------------
    /**
     *  \brief Conjugates the Ofs object. The conjugate of the serie
    ...
    /**
     *  \brief  An operation. Applies the time derivative with pulsation \f$ \omega = n \f$ directly to this.
     *  \param  n: reference to the pulsation
     ...
     *  \param  b: a reference to an Ofs object
     *  \return \c true if \c this = \c b (same order and same coefficients at every position). \c false otherwise.
     */
     ...
     *  \return a \c double containing \f$ \sum \limits_{k = -J}^{J} |c_k| \f$ for the serie \f$ F_s = \sum \limits_{k = -J}^{J} c_k e^{i k \theta }\f$.
     */
    double l1norm();
    /**
 ...
 * \return \c true if the order are equals and all the coefficients, term by term. \c false otherwise.
 */
...
/**
 *  \fn     template<typename T>  cdouble sprod_expectedError(Ofs<T> const& a, Ofs<T> const& b, double const& t)
...
/**
 *  \fn     template<typename T>  cdouble smprod_expectedError(Ofs<T> const& a, Ofs<T> const& b, T const& c,  double const& t)
 *  \brief  Expected error on the product: \c this \f$ += a \times b \f$ at times t. Works only when a.order = b.order which is the default case.
 *  \param  a: a reference to an Ofs object
 *  \param  b: reference to an Ofs object
 *  \param  c: reference to an T coefficient
 ...
 **/
void initcRsR(double t, double cR[], double sR[], int ofs_order);
