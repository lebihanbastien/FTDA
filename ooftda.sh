...
if [ "$ans" == "y" ]; then
	...
	$QBTBP | $NFO2 | $PM | $COC | $DYNEQ | $FT_TEST) 
		COEFFS=($OFTS_ORDER $OFS_ORDER $REDUCED_NV $COMPTYPE $MODEL $CS $ISNORM $LI_EM $LI_SEM $PMS $MANTYPE $STORAGE)
		;;

	$PM_TEST) 
		COEFFS=($OFTS_ORDER $OFS_ORDER $REDUCED_NV $COMPTYPE $MODEL $CS $ISNORM $LI_EM $LI_SEM $PMS $MANTYPE $STORAGE ${IC[*]} $NORDERS ${ORDERS[*]})
		;;
	$COMPMAP) 
		...
		COEFFS=(${COEFFS[*]} $LI_EM $LI_SEM $PMS $MANTYPE $STORAGE)
