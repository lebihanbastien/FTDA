...
//----------------------------------------------------------------------------------------
/**
 *   \brief Compute the error via infinity or L2 norm. The infinity norm is selected
 *          for now.
 **/
double compute_error(double* eI, int nv)
{
    //------------------------------------------------------------------------------------
    //Taking the maximum (infinity norm)
    //------------------------------------------------------------------------------------
    double eIm = fabs(eI[0]);
    for(int p = 1; p < nv; p++) if(eI[p] > eIm) eIm = fabs(eI[p]);

    //------------------------------------------------------------------------------------
    // OR Taking the L2 norm
    //------------------------------------------------------------------------------------
    //    double eIm = eI[0]*eI[0];
    //    for(int p = 1; p < nv; p++) eIm += eI[p]*eI[p];
    //    eIm = sqrt(eIm);
...
string nameFile(Pmap& pmap, int method)
{
    ...
    //Final name
    filename = filename+"Energy_"+ssHv+"_order_"+ssorder+"_ofs_"+ssofs_order+"_proj_"+ssProj+"_max_events_"+ssMaxEvents+smethod;
            ...
            if(status == GSL_SUCCESS)
            {
                ...
                if(status == GSL_SUCCESS)
                {
                    ...
                    #pragma omp critical
                    {
                        ...
                        orbit_pmap_fprint_bin(&orbit, filename, 1);
                    }
...
void pmap_precision(Pmap& pmap, int append, bool isPar)
{
    ...
    //If the energy makes sense, put it in the filename
    if(pmap.dHv > 0) filename +=+"_hmax_"+ssEnergy;
...
void pmap_invariance_error_random(Pmap& pmap, int append, bool isPar, double hzmax)
{
    ...
    //Normalisation (only EML2 case for now)
    for(int i = 0; i <= pmap.gsize; i++)
    {
        //s1 & s3
        ...
        //s2 & s4
        randseq[i][1] = 5e-1*(pmap.gmin*(1-randseq[i][1]) + pmap.gmax*randseq[i][1]);
        randseq[i][3] = 5e-1*(pmap.gmin*(1-randseq[i][3]) + pmap.gmax*randseq[i][3]);
    }
...
void pmap_invariance_error_random(Pmap& pmap, int append, bool isPar, double hzmax, int km[], int nkm)
{
    ...
    for(int m = 0; m < nkm; m++)
    {
        ...
        string filename;
        string type = "rand";
        filename = F_PRINT+"eIm_"+type+"_ofs_"+ssOfs+"_order_"+ssOrder+"_PhD";
        if(!append) header_fprint_bin(filename);
        cout << "Data stored in " << filename << endl;
        ...
        for(int i = 0; i <= pmap.gsize; i++)
        {
            ...
            if(status == GSL_SUCCESS)
            {
                ...
                #pragma omp critical
                {
                    ...
                    //--------------------------------------------------------------------
                    cout << std::noshowpos << resetiosflags(ios::scientific)  << setprecision(4);
                    if(label%((int)floor(numberOfOrbits/10)) == 0)
                    {
                        percent = (double) label/numberOfOrbits*100;
                        std::cout << "\r" << percent << "% completed: ";
                        std::cout << std::string(floor(0.1*percent), '|') << endl;
...
void pmap_invariance_error_random_planar(Pmap& pmap, int append, bool isPar, double hzmax, int km[], int nkm)
{
    ...
    for(int m = 0; m < nkm; m++)
    {
        ...
        string filename;
        string type = "rand";
        filename = F_PRINT+"eIm_"+type+"_ofs_"+ssOfs+"_order_"+ssOrder+"_planar_PhD";
        if(!append) header_fprint_bin(filename);
        cout << "Data stored in " << filename << endl;
...
void pmap_energy_3d(Pmap& pmap, int append, bool isPar, double hzmax)
{
    ...
    double* grid = dvector(0,  pmap.gsize);
    init_grid(grid, pmap.gmin, pmap.gmax, pmap.gsize);
    double numberOfOrbits = pow(1.0+pmap.gsize, 4.0);
    ...
    for(int i = 0; i <= pmap.gsize; i++)
    {
        ...
        for(int j = 0; j <= pmap.gsize; j++)
        {
            ...
            double* sti = dvector(0, 3);
            sti[0] = grid[i];
            sti[1] = 0.5*grid[j];  //for maxvalue = 60, s2 = 15
            sti[2] = grid[k];
            sti[3] = 0.5*grid[l];
    ...
    else
    {
        ...
        for(int i = 0; i <= orbit->last_indix; i++)
        {
            ...
            if(orbit->z0_mat[5][i] > 0)
            {
                ...
                //5. dHz
                res = orbit->hz[i] - orbit->pmap->H0;
                myfile.write((char*) &res, sizeof(double));

                //6. dHw
                res = orbit->hw[i] - orbit->pmap->H0;
                myfile.write((char*) &res, sizeof(double));
...
int orbit_compute_pmap(Orbit* orbit, int method)
{
    ...
    case SINGLE_INT:
    {
        //Reset @crossing
        //--------------------------------
        if(orbit->pmap->projFreq == -1) status = single_pmap(orbit); //if the projection frequency is -1, it means no projection!
        else status = single_pmap_proj(orbit);
